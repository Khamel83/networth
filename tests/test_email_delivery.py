...
def test_disabled_pairing_batch_queues_pending_rows_without_provider_call():
    from api.email_delivery import deliver_batch

    rows = _message_rows()
    queued_rows = [dict(rows[0], id='ledger-1', delivery_status='pending')]
    table = _table_with_insert(rows=queued_rows)
    provider = Mock()
...
def test_reconciliation_query_returns_pending_unknown_and_failed_rows():
    ...
    rows = [
        ...
        {'message_key': 'failed-1', 'delivery_status': 'failed'},
    ]
    table = Mock()
...
def test_pairing_reconciliation_rebuilds_from_assignments_without_provider_call(monkeypatch):
    from api.email import _rebuild_reconciliation_messages

    assignment_query = Mock()
    assignment_query.select.return_value = assignment_query
    assignment_query.in_.return_value = assignment_query
    assignment_query.eq.return_value = assignment_query
    assignment_query.execute.return_value = SimpleNamespace(
        data=[{
            'id': 'assignment-1',
            'player1_id': 'player-1',
            'player2_id': 'player-2',
    ...
    player_query = Mock()
    player_query.select.return_value = player_query
    player_query.in_.return_value = player_query
    player_query.execute.return_value = SimpleNamespace(
        data=[
            {
                'id': 'player-1',
                'name': 'One',
            ...
            },
            {
                ...
                'phone': '555-0102',
                'avail_weekend_late': True,
            },
        ],
        error=None,
    )
    table = Mock(side_effect=[assignment_query, player_query])
    monkeypatch.setattr('api.supabase_http.table', table)

    messages, error = _rebuild_reconciliation_messages(
        'generate_pairings',
        'August 2026',
        [{
            'message_key': 'generate_pairings:August 2026:assignment-1',
            'template': 'match_assignment',
            'recipient_emails': ['one@example.com', 'two@example.com'],
        }],
    )

    assert error is None
    assert messages[0]['to'] == ['one@example.com', 'two@example.com']
    assert messages[0]['reply_to'] == 'one@example.com'
    assert "One, meet Two - You're matched for August 2026!" in messages[0]['subject']
    assert 'Weekdays: 9-5' in messages[0]['html']
    assert 'Weekends: after 5pm' in messages[0]['html']
