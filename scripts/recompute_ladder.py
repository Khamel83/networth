#!/usr/bin/env python3
"""
Fixed version of recompute_ladder.py that properly handles None values.
"""
...
def clean_data_for_toml(data):
    """Recursively remove None values from data structure for TOML serialization."""
    if isinstance(data, dict):
        return {k: clean_data_for_toml(v) for k, v in data.items() if v is not None}
    elif isinstance(data, list):
        return [clean_data_for_toml(item) for item in data]
    else:
        return data
...
class LadderComputer:
    ...
    def save_ladder(self, ladder):
        ...
        # Clean data of None values
        clean_ladder = clean_data_for_toml(ladder)
